...
export default function Inventory() {
  ...
  const TabButton = ({ id, name, shortcut, icon }: TabButtonProps) => (
    <TabsTrigger
      value={id}
      className={`text-sm font-medium flex flex-col gap-2 transition-all duration-300 w-[72px] min-w-[72px] ${currentTab === id ? "bg-gradient-to-r from-[#F0F9FF] to-[#1E3A8A] text-white shadow-[0_0_20px_rgba(30,58,138,0.6)]" : "hover:opacity-80 hover:text-muted-foreground hover:bg-gradient-to-r hover:from-[#F0F9FF] hover:to-[#1E40AF] hover:text-white"}`}
    >
      {id === "inventory" ? (
        <div className={`w-[40px] h-[40px] rounded-[14px] bg-background flex items-center justify-center ${currentTab === id ? "shadow-[0_0_0_4px_rgba(0,123,255,0.4)]" : ""
          }`}>
          <CogIcon className="h-[44px] w-[44px] text-foreground" />
        </div>
      ) : (
        <div className={`w-[50px] h-[50px] rounded-[14px] ${currentTab === id ? "shadow-[0_0_0_6px_rgba(0,123,255,0.4)]" : ""
          }`}>
          <img src={getLogoPath(icon)} alt={`${name} icon`} />
        </div>
      )}
      <span className={`${currentTab === id ? "text-white" : "opacity-50"} w-full text-center text-wrap hyphens-auto break-words whitespace-normal px-1`}>
        {name}
      </span>
    </TabsTrigger>
  );
