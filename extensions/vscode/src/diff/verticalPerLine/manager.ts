  ...
  async streamEdit(
    ...
    try {
      ...
      if (error.message.includes("401")) {
        vscode.window
          .showErrorMessage(
            `Dropstone access requires login: Fetch Error: Invalid response body while trying to fetch Dropstone Server`,
            "Login to Dropstone",
          )
          .then((selection) => {
      ...
      } else if (error.message.includes("403") && (error.message.includes("Dropstone") || error.message.includes("Access denied") || error.message.includes("Authentication token required") || error.message.includes("Token expired"))) {
        vscode.window
          .showErrorMessage(
            `Dropstone authentication required: ${error.message}`,
            "Login to Dropstone",
          )
          .then((selection) => {
            if (selection === "Login to Dropstone") {
