...
function getPearAISettingsDir() {
    ...
    if (platform === 'win32') {
        return path.join(process.env.APPDATA || '', 'dropstone', 'User');
    } else if (platform === 'darwin') {
        return path.join(os.homedir(), 'Library', 'Application Support', 'dropstone', 'User');
    } else {
        return path.join(os.homedir(), '.config', 'dropstone', 'User');
    }
}
