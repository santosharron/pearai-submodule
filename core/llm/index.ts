    ...
    const customFetch = async (input: URL | RequestInfo, init: any) => {
      try {
        ...
        if (!resp.ok) {
          ...
          // Handle Dropstone authentication errors
            ...
            try {
              ...
              if (errorData.error &&
                  (errorData.error.includes("Token expired") ||
                   ...
                   errorData.authenticate)) {

                console.log('Dropstone authentication expired, clearing token and triggering re-authentication');

                // Clear the API key/token for this LLM instance
                this.apiKey = undefined;

                // Create a special error type that can be handled by the extension
                const authError = new Error(
                  `HTTP ${resp.status} ${resp.statusText} from ${resp.url}\n\n${text}`,
                );
                (authError as any).isDropstoneAuthError = true;
                (authError as any).dropstoneError = errorData.error;

                throw authError;
              }
            } catch (parseError) {
              // If JSON parsing fails, fall through to regular error handling
              if ((parseError as any).isDropstoneAuthError) {
                throw parseError;
              }
            }
          }
        ...
        // Handle Dropstone authentication errors specially
